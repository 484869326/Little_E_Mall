...
const localShoppingList = reactive([] as any[]);
const myStore = useMyStore();
const shoppingStore = useShoppingStore();
const { shoppingList, isCheckAll, totalCheckPrice, totalCheck, total, totalPrice } =
...
const debounceFn = debounce(
  async (fnName, ...args) => {
    const res = await (shoppingStore as any)[fnName](...args);
    return res;
  },
  300,
  false
...
function handleChangeCheckedClick(item: any) {
  ...
  const price: any = parseFloat(item.good.price[item.type]).toFixed(2);
  ...
  debounceFn("fetchChangeChecked", id, checked, userInfo.value.id).then((res: any) => {
    isCheckAll.value = res.data;
  });
}
//改变数量
function handleChangeNumClick(event: any, item: any, index: any) {
  const id = item.shoppingId;
  const num = item.num;
...
//全选的
function handleCheckAllClick(checkAll: any) {
...
function handleDeleteClick(item: any) {
  const id = item.shoppingId;
  const price = (parseFloat(item.good.price[item.type]).toFixed(2) as any) * item.num;
...
//关闭筛选类型价格变化
function handleCloseClick(isChecked: any, price: any) {
