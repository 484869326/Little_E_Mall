<template>
  <view class="search-content">
    <view class="search-history" v-if="historyList.length !== 0">
      ...
      <view class="history-list" :style="{ 'max-height': historyListHeight[range] }">
        <view class="history-list-box">
          <template v-for="item in historyList" :key="item">
            ...
            <view class="history-item" v-else @click="$emit('searchClick', item)">
              {{ item }}
            </view>
          </template>
        </view>
...
const localHistoryList = reactive([] as any[]);
...
function initDom(isTrue: boolean) {
