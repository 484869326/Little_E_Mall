<template>
  <view class="order-manage">
    <navigation url="/pages/my/my" :title="`订单管理`" :isTab="true"></navigation>
            ...
            <view class="son-good">
              ...
              <view class="good-name">
                {{
                  sonItem.goodName +
                  " " +
                  (sonItem.type as string).split(",")[(item.type as any)[sonIndex]] +
                  " " +
                  (sonItem.color as string).split(",")[(item.color as any)[sonIndex]]
                }}</view
              >
              <view class="good-liquidation">
                <view class="good-price"
                  >￥{{
                    (sonItem.price as string).split(",")[(item.type as any)[sonIndex]]
                  }}</view
                >
                <view class="good-num">x{{ item.num.split(",")[sonIndex] }}</view>
              </view>
            </view>
          </template>
        </view>
...
const { orderList, orderPage } = storeToRefs(goodStore);
...
//确认收货按钮
function handleConfirmClick(condition: string, orderId: number, index: number) {
  if (condition === "购买成功") {
    useTips(tipsRef, "未发货");
    return;
  }
