...
async function handleSavaClick() {
  if (!formData.addressName) {
    useTips(tipsRef, "收货人姓名不能为空");
    return;
  }
  if (!formData.addressPhone) {
    useTips(tipsRef, "收货人手机号不能为空");
    return;
  }
  if (!phoneReg.test(formData.addressPhone)) {
    useTips(tipsRef, "收货人手机号格式不对");
    return;
  }
  if (!formData.addressRegion) {
    useTips(tipsRef, "收货人收货地区不能为空");
    return;
  }
  if (props.index) {
  ...
  if (res !== "更新成功") {
    useTips(tipsRef, "系统异常,请稍后再试");
    return;
  }
  userInfo.value.city = city;
  emit("save");
}
function handleChange({ detail }: { detail: { value: any[] } }) {
  const value = detail.value;
  formData.addressRegionText = value.map((item: any) => item.text).join("/");
}
</script>
